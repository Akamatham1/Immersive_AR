...
- `.claudeignore` excludes Unity's generated/cache directories (`Library`, `Temp`, `.utmp`, `Logs`, `UserSettings`) and binary asset types from Claude's context — `.utmp` in particular is a Gradle/CMake build-cache directory. It was historically tracked in git but is now gitignored (along with `.idea/`, `.vscode/`, and `.DS_Store`), so it only exists locally and should never be treated as source.
- `Assets/Resources/VuforiaConfiguration.asset` holds the Vuforia license key and is gitignored. Each developer supplies their own key locally. Never re-add it to version control.
