...
**Video playback.** `Assets/LivePlayer.cs` configures a `VideoPlayer` to stream from a hardcoded HLS URL (`videoplayback`/live stream with a signed, time-limited token — this URL will expire and need regenerating). `ARVideoPlayerController.cs` and `PlayPauseToggle.cs` provide manual play/pause controls; `TouchRotate.cs` lets the user rotate the tracked content with touch/mouse drag.
